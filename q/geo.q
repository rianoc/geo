...
pl:{raze{select[x]lat,lon,trk,time from geo}each flip deltas geo.cid binr/:x}
lu:{[x;y]select from pl rect . x where all(lat;lon;time)within'(x,enlist y)}

.z.ws:{r:.j.k x;p:(neg[d];d:r[4])+/:2#r;t:"D"$r 2 3;neg[.z.w].j.j flip lu[p;t]}
